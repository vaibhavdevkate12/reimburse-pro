                          ...
                          <td style={{ textAlign: 'center' }}>
                            {item.proof_path ? (
                              <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                                <button 
                                  className="btn btn-secondary"
                                  onClick={() => setPreviewUrl(`/api/file?url=${encodeURIComponent(item.proof_path || '')}`)}
                                >
                                  Preview
                                </button>
                                <a 
            ...
            <div key={`proof-${idx}`} className="print-proof-page" style={{ pageBreakBefore: 'always', breakBefore: 'page' }}>
              ...
              <div className="proof-content">
                {item.proof_path.toLowerCase().endsWith('.pdf') ? (
                  <PDFRenderer url={`/api/file?url=${encodeURIComponent(item.proof_path)}`} />
                ) : (
                  <img src={`/api/file?url=${encodeURIComponent(item.proof_path)}`} alt={`Proof ${idx + 1}`} style={{ maxWidth: '100%', maxHeight: '19cm', width: 'auto', display: 'block', margin: '0 auto', border: '1px solid #ddd' }} />
                )}
              </div>
            </div>
          ))}
